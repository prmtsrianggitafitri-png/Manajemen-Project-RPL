...
<head>
  ...
  <link
    href="https://fonts.googleapis.com/css2?family=Roboto:ital,wght@0,100;0,300;0,400;0,500;0,700;0,900&family=Poppins:ital,wght@0,100;0,200;0,300;0,400;0,500;0,600;0,700;0,800;0,900&family=Nunito:ital,wght@0,200;0,300;0,400;0,500;0,600;0,700;0,800;0,900&display=swap"
    rel="stylesheet">
  ...
  <div class="container-fluid container-xl d-flex align-items-center justify-content-between custom-container">
    ...
    <nav id="navmenu" class="navmenu m-0 p-0 d-none d-xl-flex">
      <ul class="d-flex align-items-center gap-4 mb-0 list-unstyled">
        <li><a href="{{ url('/') }}"
            class="{{ Request::is('/') ? 'active custom-nav-link' : 'custom-nav-link-normal' }}">Beranda</a></li>
        <a href="{{ route('mahasiswa.publik') }}"
          class="{{ (request()->routeIs('mahasiswa.publik') || (request()->routeIs('mahasiswa.profil') && request()->query('from') == 'mahasiswa')) ? 'active' : '' }}">
          Mahasiswa
        </a>

        <a href="{{ route('mahasiswa.alumni') }}"
          class="{{ (request()->routeIs('mahasiswa.alumni') || (request()->routeIs('mahasiswa.profil') && request()->query('from') == 'alumni')) ? 'active' : '' }}">
          Alumni
        </a>
      </ul>
    ...
    <div class="header-right d-flex align-items-center gap-3 m-0 p-0">
      <form
        action="{{ request()->routeIs('mahasiswa.publik') ? route('mahasiswa.publik') : (request()->routeIs('mahasiswa.alumni') ? route('mahasiswa.alumni') : url()->current()) }}"
        method="GET" class="d-flex align-items-center m-0">
        <div class="search-bar position-relative d-none d-md-block">
          <input type="text" name="search" placeholder="Search..." value="{{ request('search') }}"
            class="form-control ps-4 pe-6 py-2 custom-search-input">
          <button type="submit" class="btn p-0 position-absolute end-0 top-50 translate-middle-y pe-3"
      ...
      @auth
        <div class="dropdown">
          <button class="btn dropdown-toggle d-flex align-items-center gap-2 custom-btn-login" type="button" id="userMenu"
